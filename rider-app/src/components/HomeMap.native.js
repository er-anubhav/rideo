...
const sanitizeRoutePath = (routePath, referencePoints) => {
...
const buildHtml = ({ mapplsRestKey, interactive }) => `<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8" />
    <meta
...
const buildPayload = ({
    ...
    const driverMarker = driverLocation
        ? {
            lat: Number(driverLocation.lat ?? driverLocation.latitude),
            lng: Number(driverLocation.lng ?? driverLocation.longitude),
        }
        : null;
    ...
    const fallbackPath = routePath.length > 1
        ? routePath
        : [pickupMarker, driverMarker, dropMarker].filter(Boolean);
    const sanitizedRoutePath = sanitizeRoutePath(
        fallbackPath,
        [pickupMarker, driverMarker, dropMarker],
    );
...
const HomeMap = ({
    ...
    const html = useMemo(
        () => buildHtml({ mapplsRestKey, interactive }),
        [interactive, mapplsRestKey],
    );
    const payload = useMemo(
        () => buildPayload({ region, location, destination, driverLocation, routeCoordinates }),
        [destination, driverLocation, location, region, routeCoordinates],
    );
